...
TEST(QuickSortTests, TestArrayOfIntegers) {
...
TEST(QuickSortTests, TestRandomArrayOfIntegers) {
  srand(time(NULL));
...
TEST(QuickSortTests, TestArrayOfStrings) {
  ...
  QuickSort<std::string>::Sort(elements);
...
TEST(QuickSortTests, TestArrayOfDoubles) {
  ...
  QuickSort<double>::Sort(elements);
...
TEST(QuickSortTests, TestArrayOfIntegersInDecendingOrder) {
  std::vector<int> elements{ 5,3,7,8,0,4,2 };
  auto elements_copy{ elements };
  ...
  QuickSort<int>::Sort(elements, [](int a, int b) {return a > b; });
